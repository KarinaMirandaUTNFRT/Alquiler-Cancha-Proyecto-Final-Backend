...
export const reglasCategoriaProducto = [
  body("nombre")
    ...
    .withMessage(
      "El nombre de la categoria debe contener entre 3 y 100 caracteres",
    )
    .custom(async (valor, { req }) => {
      const categoriaBuscada = await CategoriaProducto.findOne({
        nombre: valor,
      });
...
export const validacionCategoria = [
  ...reglasCategoriaProducto.map((regla) =>
    regla.notEmpty().withMessage("Este campo es obligaotio"),
  ),
  resultadoValidacion,
];

export const validacionCategoriaPatch = [
  ...reglasCategoriaProducto.map((regla) =>
    regla.optional({ values: "falsy" }),
  ),
  resultadoValidacion,
];

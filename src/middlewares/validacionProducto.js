...
export const reglasProducto = [
  body("nombreProducto")
    ...
    .custom(async (valor, { req }) => {
      ...
      throw new Error(
        "El nombre del producto ingresado ya exite, dato no valido ingrese un nombre distinto al ingresado",
      );
    }),
  body("precio")
    .isNumeric()
...
export const validacionProducto = [
  ...reglasProducto.map((regla) =>
    regla.notEmpty().withMessage("Este campo es obligaotio"),
  ),
  resultadoValidacion,
];

export const validacionProductoPatch = [
  ...reglasProducto.map((regla) => regla.optional({ values: "falsy" })),
  resultadoValidacion,
];
